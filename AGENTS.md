...
- **Deploy pipeline: `npm run build` → `npx wrangler deploy`** (run locally). The built output lands in `.output/server/` and wrangler reads `.output/server/wrangler.json` generated by Nitro. No CI pipeline exists.
- **`VITE_*` env vars are baked in at build time** — Vite reads `.env.production` from the project root during `npm run build` and inlines the values into the Worker bundle. Setting vars in Cloudflare's dashboard has no effect on them. `.env.production` is gitignored; copy from `.env.example` and fill in real values before building.
